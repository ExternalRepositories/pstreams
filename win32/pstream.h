/* $Id: pstream.h,v 1.1.2.1 2004/09/27 10:05:56 redi Exp $
...
public:
	...
	/// Constructor that initialises the buffer with @a program and @a argv.
											basic_pstreambuf(const Program& program,
															 const std::vector<std::string>& argv,
															 pmode mode);
	...
	/// Initialise the stream buffer with @a program and @a argv.
	basic_pstreambuf*						open(const Program& program,
												 const std::vector<std::string>& argv,
												 pmode mode);
...
template <typename CharT, typename Traits = std::char_traits<CharT> >
class pstream_common : virtual public std::basic_ios<CharT, Traits>, public pstreams {
protected:
	...
	/// Constructor that initialises the stream by starting a process.
											pstream_common(const Program& program,
														   const std::vector<std::string>& argv,
														   pmode mode);
	...
	/// Start a process.
	void									do_open(const std::string& file,
													const std::vector<std::string>& argv,
													pmode mode);
	/// Start a process.
	void									do_open(const Program& program,
													const std::vector<std::string>& argv,
													pmode mode);
public:
	/// Close the pipe.
...
template <typename CharT, typename Traits = std::char_traits<CharT> >
class basic_ipstream : public std::basic_istream<CharT, Traits>, public pstream_common<CharT, Traits> {
...
public:
	 ...
	 * arguments.
	 *
	 * @param program  a program containing the pathname of a program to execute.
	 * @param argv     a vector of argument strings passed to the new program.
	 * @param mode     the I/O mode to use when opening the pipe.
	 * @see   do_open(const std::string&, const std::vector<std::string>&, pmode)
	 */
	basic_ipstream(const Program& program, const std::vector<std::string>& argv, pmode mode = std::ios_base::in) :
		istream_type(NULL),
		pbase_type(program, argv, mode)
	{
		unsetf(ios_base::skipws);
	}
	/**
	 ...
	 * @a argv and opens pipes to the process as given by @a mode.
	 *
	 * @param program  a string containing the pathname of a program to execute.
	 * @param argv     a vector of argument strings passed to the new program.
	 * @param mode     the I/O mode to use when opening the pipe.
	 * @see   do_open(const std::string&, const std::vector<std::string>&, pmode)
	 */
	void
	open(const Program& program, const std::vector<std::string>& argv, pmode mode = std::ios_base::in) {
		this->do_open(program, argv, mode);
	}
...
template <typename CharT, typename Traits = std::char_traits<CharT> >
class basic_opstream : public std::basic_ostream<CharT, Traits>, public pstream_common<CharT, Traits> {
...
public:
	 ...
	 * @see   do_open(const std::string&, const std::vector<std::string>&, pmode)
	 */
	basic_opstream(const Program& program, const std::vector<std::string>& argv, pmode mode = std::ios_base::out) :
		ostream_type(NULL),
		pbase_type(program, argv, mode)
	{
	}
	 ...
	 */
	void
	open(const Program& program, const std::vector<std::string>& argv, pmode mode = std::ios_base::out) {
		this->do_open(program, argv, mode);
	}
};
 ...
 */
template <typename CharT, typename Traits = std::char_traits<CharT> >
class basic_pstream : public std::basic_iostream<CharT, Traits>, public pstream_common<CharT, Traits> {
...
public:
	 ...
	 * @see   do_open(const std::string&, const std::vector<std::string>&, pmode)
	 */
	basic_pstream(const Program& program,
				  const std::vector<std::string>& argv,
				  pmode mode = std::ios_base::in | std::ios_base::out) :
		iostream_type(NULL),
		pbase_type(program, argv, mode)
...
public:
	 ...
	 * @see   do_open(const std::string&, const std::vector<std::string>&, pmode)
	 */
	basic_rpstream(const Program& program,
				   const std::vector<std::string>& argv,
				   pmode mode = std::ios_base::in | std::ios_base::out) :
		ostream_type(NULL),
		istream_type(NULL),
...
template <typename C, typename T>
inline
basic_pstreambuf<C,T>::basic_pstreambuf(const Program& program, const std::vector<std::string>& argv, pmode mode) :
#if		defined(OS_UNIX)
	ppid_(0),
	wpipe_(-1),
#elif	defined(OS_WIN32)
...
basic_pstreambuf<C,T>::open(const std::string& command, pmode mode) {
	...
	if (!is_open()) {
		...
		switch (fork(mode)) {
			...
			default:
			try {
				Program program = Program(command);
				process = Process(program, childstdin.in(), childstdout.out(), childstderr.out());
...
template <typename C, typename T>
basic_pstreambuf<C, T>*
basic_pstreambuf<C,T>::open(const Program& program, const std::vector<std::string>& argv, pmode mode) {
	basic_pstreambuf<C, T>*	ret	= NULL;
...
//FA	rpipe_[rsrc_out] = rpipe_[rsrc_err] = -1;
...
basic_pstreambuf<C,T>::create_buffers(pmode mode) {
	...
	if (mode & pstderr) {
		...
		if (rsrc_ == rsrc_err)
			this->setg(rbuffer_[rsrc_err] + pbsz, rbuffer_[rsrc_err] + pbsz, rbuffer_[rsrc_err] + pbsz);
	}
}
...
#if		defined(OS_UNIX)
//FA if (rpipe_[src].valid(Pipe::ReadSide)) {
	if (rpipe_[src] >= 0) {
#elif	defined(OS_WIN32)
...
//FA	return wpipe().valid() ? ::write(wpipe().out(), s, n * sizeof(char_type)) : 0;
...
template <typename C, typename T>
inline
pstream_common<C,T>::pstream_common(const Program& program, const std::vector<std::string>& argv, pmode mode) :
	std::basic_ios<C, T>(NULL),
	program_(program),
	buf_()
{
	this->init(&buf_);
	do_open(program, argv, mode);
}
/**
...
template <typename C, typename T>
inline void
pstream_common<C,T>::do_open(const std::string& file, const std::vector<std::string>& argv, pmode mode) {
 ...
 * opens pipes to the process as given by @a mode.
 *
 * @param program a string containing a shell command.
 * @param mode    the I/O mode to use when opening the pipe.
 * @see   basic_pstreambuf::open()
 */
template <typename C, typename T>
inline void
pstream_common<C,T>::do_open(const Program& program, const std::vector<std::string>& argv, pmode mode) {
	if (!buf_.open((program_ = program), argv, mode)) {
		this->setstate(std::ios_base::failbit);
	}
}
